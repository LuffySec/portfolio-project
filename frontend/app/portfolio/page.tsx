...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    // FIX: Renamed internal function to loadProjects to avoid shadowing the API import
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading
            ...
            : filteredProjects.map((project) => (
                <div
                  key={project.id}
                  className="flex flex-col h-full rounded-2xl bg-gray-900/50 border border-gray-800/50 overflow-hidden hover:border-indigo-500/50 transition-colors duration-300 group"
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-gray-800/50 mt-auto">
                      <a
                        ...
                        className="text-sm font-semibold text-white hover:text-indigo-400 transition-colors duration-300 flex items-center gap-2"
                      >
                        Live Demo <span className="text-xs">↗</span>
                      </a>
                      <a
                        href={project.githubUrl}
                        className="text-sm font-semibold text-gray-400 hover:text-white transition-colors duration-300 flex items-center gap-2"
                      >
